...
- 在**代理辅助进化算法（Surrogate assisted EA）** 中，使用一个**代理函数 f(s)** 来代替真实的目标函数 **f(s)**，以**加速**对种群成员的评估（综述请参见 [Jin11]）。这与**贝叶斯优化**（第6.6节）中使用的**响应面模型**类似，**不同之处在于**它不处理**探索-利用权衡（explore-exploit tradeoff）**。
