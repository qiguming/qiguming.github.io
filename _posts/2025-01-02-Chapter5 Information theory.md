...
在语言模型中，我们通常在预测下一个单词时，以前面的单词作为已知条件。例如，在一个二元语法模型（bigram model）中，我们使用如下形式的二阶马尔可夫模型：$$p(x_n \mid x_{n-1})$$。我们定义语言模型的**分支因子**（branching factor）为模型预测的下一个可能单词的数量。例如，假设模型以均等的可能预测每个单词，即 $$p(x_n \mid x_{n-1}) = 1/K$$，其中 $K$ 是词汇表中的单词总数。那么，困惑度（perplexity）为：$$((1 / K)^N)^{-1 / N}=K$$。如果某些单词的可能性更高，并且模型能够正确反映这种情况，那么模型的困惑度将低于$K$。然而，考虑到 $$\mathbb{H}(p^*) \leq \mathbb{H}_{c e}(p^*, q)$$，因此我们无法将困惑度降低到 $$2^{-\mathbb{H}\left(p^*\right)}$$以下。
