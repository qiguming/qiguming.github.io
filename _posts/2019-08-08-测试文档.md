...
在本节，我们介绍了著名的**反向传播算法**（backpropagation algorithm），该算法可用于计算损失函数关于网络每一层参数的梯度，该损失函数应用于网络的输出。 然后，可以将此梯度传递给基于梯度的优化算法，如我们在第13.4节中讨论的那样。
反向传播算法最初是在[BH69]中发现的，同时在[Wer74]中被独立发现的。 但是，正是[RHW86]使该算法引起了“主流” ML社区的注意。 有关更多历史记录，请参见Wikipedia page3。
我们首先假设计算图是一个简单的堆叠层的线性链，就像在MLP中一样。 在这种情况下，反向传播等效于重复使用链式法则（参见等式（B.42））。 但是，如我们在第13.3.4节中所讨论的，该方法可以推广到任意有向无环图（DAG）。 该过程的一般性版本通常被称为**自动微分**（automatic differentiation, autodiff）。

### 13.3.1 前向与反向可微

考虑一个形式为 $\mathbf{o}=\mathbf{f}(\mathbf{x})$，其中 $\mathbf{x}\in \mathbb{R}^n$, $\mathbf{o}\in \mathbb{R}^m$。假设 $\mathbf{f}$ 定义为多个函数的组合：
$$
\mathbf{f}=\mathbf{f}_4 \circ \mathbf{f}_3 \circ \mathbf{f}_2 \circ \mathbf{f}_1 \tag{13.27}
$$
其中 $\mathbf{f}_1:\mathbb{R}^n \rightarrow \mathbb{R}^{m_1}$, $\mathbf{f}_2:\mathbb{R}^{m_1} \rightarrow \mathbb{R}^{m_2}$, $\mathbf{f}_3:\mathbb{R}^{m_2} \rightarrow \mathbb{R}^{m_3}$, $\mathbf{f}_4:\mathbb{R}^{m_3} \rightarrow \mathbb{R}^{m}$。 中间的步骤需要计算 $\mathbf{o}=\mathbf{f}(\mathbf{x})$, 即 $\mathbf{x}_2=\mathbf{f}_1(\mathbf{x})$, $\mathbf{x}_3=\mathbf{f}_2(\mathbf{x}_2)$, $\mathbf{x}_4=\mathbf{f}_3(\mathbf{x}_3)$, $\mathbf{o}=\mathbf{f}_4(\mathbf{x}_4)$。

我们可以使用链式法则计算雅各比矩阵 $\mathbf{J}_\mathbf{f}(\mathbf{x})=\frac{\partial \mathbf{o}}{\partial \mathbf{x}}\in \mathbb{R}^{m\times n}$：
