...
$$
当策略 $\pi_k$ 相对于其自身的价值函数 $V_{\pi_k}$ 是贪心的，算法在第 $k$ 次迭代停止。此时，该策略是最优的。由于确定性策略最多有  $|\mathcal{A}|^{|\mathcal{S}|}$种，且每次迭代都会严格改进策略，因此算法必定在有限次迭代后收敛。
...
$$
常数 $(1-\gamma)$ 将 $d$ 归一化为一个合法的分布，使其总和为$1$。通过这种对 d*d* 的解释，式（34.93）中的目标函数就是归一化占用分布下的平均每步奖励。一旦找到最优解 $d_*$，最优策略就可以立即由 $\pi_*(a \mid s)=d_*(s, a) / \sum_{a^{\prime}} d_*\left(s, a^{\prime}\right)$ 获得。
