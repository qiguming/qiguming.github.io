...
> 本章介绍统计决策理论，其背后动机在于，已知外部世界的潜在状态后，我们如何选择最优的行动，从而实现外部奖励的最大化。
>
...
贝叶斯推断提供了根据观测值 $\boldsymbol{X}=\boldsymbol{x}$ 更新关于隐变量 $H$ 信念的最佳方式——计算后验概率分布 $p(H\mid\boldsymbol{x})$。然而，无论如何，我们都需要将信念状态转换成影响外部世界的**行动**（actions）。那么，我们又该如何决定哪一种行动是最优呢？这便是**决策理论**（decision theroy）需要回答的问题。本章，我们将简单介绍相关的内容。

### 34.1.1 基础知识

在**统计决策理论**（statistical deciosn theory）框架中，存在一个**智能体**（agent）或者决策者，它需要在一系列可选决策 $a\in\mathcal{A}$ 中选择一个**行动**（action），作为应对某个观测或数据 $\boldsymbol{x}$ 的响应，此处假设 $\boldsymbol{x}$ 采样自智能体外部的某个环境；我们将该环境所处的状态表示为某个隐变量或者未知变量 $h\in\mathcal{H}$ ——即**本质状态**（state of nature）。最后，假设存在某个**损失函数**（loss function）$\ell(h, a)$，它反应了本质状态为 $h$ 时执行行动 $a$ 所导致的损失。我们的目标是定义一种**策略**（policy）$a=\delta(\boldsymbol{x})$——又被称为**估计量**（estimator）或**决策程序**（decision procedure），它决定了不同观测下需要采取的行动，从而最小化期望损失——又被称为**风险**（risk）：
...
上式的关键在于如何定义期望。接下来介绍频率学派和贝叶斯学派两种定义视角。

### 34.1.2 频率学派决策理论

**频率学派决策理论**（frequentist decision theory）假设本质状态 $h$ 是固定但未知的量，并将数据 $\mathbf{X}$ 视为随机变量。所以期望的计算是基于数据的，即**频率风险**（frequentist risk）：
...
其背后思想是一个好的策略在不同的数据集上都应表现为低风险。

遗憾的是，本质状态 $h$ 是未知的，所以上式无法计算。某些方案假设 $h$ 服从某种先验分布，然后计算**贝叶斯风险**（Bayes risk），又被称为 **综合风险**（integrated risk）:
...
当然，引入先验分布的做法与频率学派本身是不相容的。因此，我们可以使用**最大风险**（maximum risk）替代。定义为：
...
另一种等效表述如下：定义**效用函数**（utility function）$U(h, a)$，用以衡量在每种可能状态下采取不同行动的满意度。如果令 $U(h, a)=-\ell(h, a)$，最优策略可以定义为：
...
<img src="/assets/img/figures/book2/34.1.png" alt="image-20260102103414285" style="zoom:50%;" />
...
更一般地，可以证明，任何一个**可接受策略**（admissable policy）¹ 都等价于基于某个（可能为非正常）先验分布的贝叶斯策略，这一结果被称为**瓦尔德定理**（Wald’s theorem）[Wal47]。（该定理更通用的版本可参见[DR21]。）**贝叶斯框架在理论上已经足够广泛，足以涵盖所有“好”的决策策略。**因此，可以说，我们“局限于”贝叶斯方法并不会损失任何可能性（尽管我们仍需检验模型的假设是否充分，这是我们在第3.9节讨论的主题）。关于这一点的进一步讨论，请参考[BS94, p448]。
...
现在假设本质状态为标量 $h\in \mathbb{R}$，对应的行动也是标量 $y\in\mathbb{R}$。对于连续状态和行动，最常见损失是 $\ell_2$ 损失，又被称为 **平方误差**（squared error）或者 **二次损失**（quadratic loss），定义为：
...
在某些场景下，如自然语言处理或计算机视觉，预期的动作是返回一个对象 $y \in \mathcal{Y}$（例如一组标签或人体姿态）。该输出对象不仅需要在给定输入 $\boldsymbol{x}$ 时是可能的，而且输出内部本身也需要保持一致。例如，假设 $\boldsymbol{x}$ 是一个音素序列，而 $\boldsymbol{y}$ 是一个单词序列。尽管在逐词的基础上，$\boldsymbol{x}$ 可能听起来更像 $\boldsymbol{y}$ = "How to wreck a nice beach"，但如果我们考虑单词序列的整体性，那么我们可能会发现（在语言模型先验下）$\boldsymbol{y}$ = "How to recognize speech" 总体上更可能成立。（参考图 34.3。）我们可以使用**结构化预测模型**（structured prediction model），例如**条件随机场**（参见第 4.4 节），来捕捉这种在给定输入情况下，输出之间的依赖关系。

除了对 $p(\boldsymbol{y} \mid \boldsymbol{x})$ 的输出之间的依赖关系进行建模，我们可能更偏好某些特定的 $\hat{\boldsymbol{y}}$，这一点可以通过损失函数 $\ell(\boldsymbol{y}, \hat{\boldsymbol{y}})$ 进行实现。例如，如图 34.3所示，我们可能不愿意轻易地假设用户在步骤 $t$ 说的是 $\hat{y}_t$ = "nudist"，除非我们对该预测非常有信心，因为错误地预测出这个词的代价可能高于其他单词。
...
（以 [Rai68] 中的例子为例）考虑为一位石油"勘探者"所面临的决策问题建立一个模型。勘探者是指钻探"野猫井"（wildcatter）的人，"野猫井"是指在尚未确认为油田的区域钻探的勘探井。

假设你必须决定是否在某个给定地点钻探一口油井。你有两种可能的行动：$d=1$ 表示钻探，$d=0$ 表示不钻探。假设存在 3 种状态：$o=0$ 表示该井是干井（无油），$o=1$ 表示是湿井（有一些油），$o=2$ 表示是饱和井（有大量油）。我们可以用决策图来表示这个问题，如图 34.4(a) 所示。
...
已知 $$\max \left(X_1, X_2\right)=\frac{X_1+X_2}{2}+\frac{ \mid X_1-X_2 \mid}{2}$$，设 $$Z=X_1-X_2$$，则 $$Z \sim \mathcal{N}(0,2)$$，我们有：$$\mathbb{E}\left[X_1+X_2\right]=0$$，$$\mathbb{E}[\mid Z \mid]=\sqrt{\frac{2}{\pi}} \cdot \sqrt{2}=\frac{2}{\sqrt{\pi}}$$。因此
...
上式假设 $\mu_j$ 已知。由于它们实际上未知，我们可以使用 $\mathbb{E}[\Pr(\pi(y_1, y_0) = 1 \mid \mu_1 < \mu_0)]$ 来计算期望错误率。根据对称性，量 $\mathbb{E}[\Pr(\pi(y_1, y_0) = 0 \mid \mu_1 > \mu_0)]$ 相同。可以证明，这两个量由下式给出：
...
为了设置高斯先验，[FB19] 使用了约 2000 个先前 A/B 测试的经验数据。对于每个测试，他们观察了每个版本页面被展示的次数，以及用户点击每个版本的总次数。基于这些数据，他们使用分层贝叶斯模型推断出 $\mu_j \sim \mathcal{N}(m = 0.68, \tau = 0.03)$。这个先验意味着期望效应大小相当小：$\mathbb{E}[\mid\mu_1 - \mu_0\mid] = 0.023$，（这与 [Aze+20] 的结果一致，他们发现对 Microsoft Bing EXP 平台的大多数更改效果可以忽略不计，尽管偶尔会有一些“大成功”。）
...
上下文老虎机模型有许多实际应用。例如，考虑一个**在线广告系统**（online advertising system）。在这种情况下，状态 $s_t$ 代表用户当前浏览网页的特征，而行动 $a_t$ 代表系统选择展示的广告。由于广告的相关性取决于网页内容，奖励函数的形式为 $R\left(s_t, a_t\right)$，因此这个问题是上下文相关的。目标是最大化期望奖励，这相当于最大化用户点击广告的期望次数；被称为**点击率**（click through rate, CTR）。（关于此应用的更多信息，可参见 [Gra+10; Li+10; McM+13; Aga+14; Du+21; YZ22]。）
...
本节讨论探索-利用权衡的最优解，令奖励函数的参数满足后验分布 $$\boldsymbol{b}_t=p\left(\boldsymbol{\theta} \mid \boldsymbol{h}_t\right)$$，其中 $$\boldsymbol{h}_t=\left\{s_{1: t-1}, a_{1: t-1}, r_{1: t-1}\right\}$$ 表示观测历史；$$\boldsymbol{b}_t$$ 被称为 **信念状态**（belief state）或**信息状态**（information state），它是历史 $$\boldsymbol{h}_t$$ 的有限充分统计量。信念状态可以使用贝叶斯法则进行确定性的更新：

$$
\boldsymbol{b}_t=\operatorname{BayesRule}\left(\boldsymbol{b}_{t-1}, a_t, r_t\right) \tag{34.55}
$$

例如，考虑与上下文无关的**伯努利赌博机**（Bernoulli bandit），其中 $p_R(r\mid a) = \text{Ber}(r\mid \mu_a)$，并且 $\mu_a = p_R(r = 1\mid a) = R(a)$ 是采取动作 $a$ 的期望奖励。假设使用一个因式分解的 Beta 先验
...
我们可以使用类似的方法处理**高斯赌博机**（Gaussian bandit），其中 $p_R(r\mid a) = \mathcal{N}(r\mid \mu_a, \sigma^2_a)$，利用第 3.4.3 节的结果。对于上下文老虎机，问题变得更加复杂。如果我们假设是一个**线性回归赌博机**（linear regression bandit）$p_R(r \mid s, a ; \boldsymbol{\theta})=\mathcal{N}\left(r \mid \boldsymbol{\phi}(s, a)^{\top} \boldsymbol{\theta}, \sigma^2\right)$，我们可以使用贝叶斯线性回归来计算闭式解下的 $p\left(\boldsymbol{\theta} \mid \mathcal{D}_t\right)$，正如第 15.2 节所讨论的。如果我们假设是一个**逻辑回归赌博机**（logistic regression bandit）$p_R(r\mid s, a; \boldsymbol{\theta}) = \text{Ber}(r\mid\sigma(\boldsymbol{\phi}(s, a)^T\boldsymbol{\theta}))$，我们可以使用贝叶斯逻辑回归来计算 $p(\boldsymbol{\theta}\mid\mathcal{D}_t)$，正如第 15.3.5 节所讨论的。如果我们有一个形式为 $p_R(r\mid s, a; \boldsymbol{\theta}) = \text{GLM}(r\mid f(s, a; \boldsymbol{\theta}))$ 的**神经网络赌博机**（neural bandit），其中 $f$ 是一个非线性函数，那么后验推断会变得更具挑战性，正如我们在第 17 章讨论的。不过，标准的技术，例如扩展卡尔曼滤波器（第 17.5.2 节），仍然可以应用。（关于如何将此方法扩展到大型 DNN，参见 [DMKM22] 的“子空间神经老虎机”方法。）
...
一种方法是使用**集中不等式**（concentration inequality） [BLM16] 来推导估计误差的高概率上界：$\mid\hat{R}_t(s, a) - R_t(s, a)\mid \leq \delta_t(s, a)$，其中 $\hat{R}_t$ 是 $R$ 的通常估计（一般是极大似然估计，MLE），$\delta_t$ 是适当选择的函数。然后通过设定 $\tilde{R}_t(s, a) = \hat{R}_t(s, a) + \delta_t(s, a)$得到**乐观奖励估计**。
...
这告诉我们在状态 $s$ 下选择行动 $a$ 然后切换到策略 $\pi$（相对于始终遵循 $\pi$ 的基线回报）所带来的收益。请注意，$A_\pi(s, a)$ 既可以是正数也可以是负数，并且由于$$V_\pi(s)=\mathbb{E}_{\pi(a \mid s)}\left[Q_\pi(s, a)\right]$$，我们有 $$\mathbb{E}_{\pi(a \mid s)}\left[A_\pi(s, a)\right]=0$$。

### 34.5.5 最优价值函数和策略

假设 $$\pi_*$$ 是某个策略，使得对于所有状态 $$s \in \mathcal{S}$$ 和所有策略 $$\pi$$，都有 $$V_{\pi_*} \geq V_\pi$$，那么它就是一个**最优策略**（optimal policy）。同一个 MDP 可能有多个最优策略，但根据定义，它们的价值函数必须相同，分别为 $$V_*$$ 和 $$Q_*$$。我们称 $$V_*$$ 为**最优状态价值函数**（optimal state-value function），称 $$Q_*$$ 为**最优行动价值函数**（optimal action-value function）。此外，任何有限 MDP 都至少存在一个确定性的最优策略 [Put94]。
...
遵循这样的最优策略可以确保智能体从任何状态开始都能获得最大的期望回报。求解 $$V_*$$、$$Q_*$$ 或 $$\pi_*$$ 的问题被称为**策略优化**（policy optimization）。相比之下，对于给定的策略 $\pi$，求解 $V_\pi$ 或 $Q_\pi$ 被称为**策略评估**（policy evaluation），这构成了强化学习领域的一个重要主题，我们将在后续章节讨论。对于策略评估，我们有类似的贝尔曼方程，只需将方程 (34.82) 和 (34.83) 中的 $$\max _a\{\cdot\}$$ 替换为 $$\mathbb{E}_{\pi(a \mid s)}[\cdot]$$ 即可。

在方程 (34.84) 和 (34.85) 中，与贝尔曼最优方程一样，我们必须对 $\mathcal{A}$ 中的所有行动取最大值，这个使价值函数 $Q_*$ 或  $V_*$ 最大化的行动被称为**贪婪行动**（greedy action）。如果 $\mathcal{A}$ 是一个小的有限集，那么寻找贪婪行动在计算上是容易的。对于高维连续空间，我们可以将 $a$ 视为一个行动序列，并逐维进行优化 [Met+17][Met+17]，或者使用无梯度优化器，例如交叉熵方法（第 6.7.5 节），正如 **QT-Opt** 方法中所使用的  [Kal+18a][Kal+18*a*]。最近，**连续行动 Q 学习**（continuous action Q-learning，CAQL，[Ryu+20][Ryu+20]）提出使用混合整数规划来解决 argmax 问题，利用了 $Q$ 网络的 ReLU 结构。我们也可以在学习了最优 $Q$ 函数后，通过训练一个策略 $$a_*=\pi_*(s)$$ 来分摊这种优化的成本。

<img src="/assets/img/figures/book2/34.13.png" alt="image-20251220132407821" style="zoom:50%;" />

#### 34.5.5.1 案例

在本节中，我们通过一个简单示例具体化价值函数等概念。考虑如图34.13(a)所示的**一维网格世界**（grid world）。其中存在5个可能的状态，$S_{T1}$ 与 $S_{T2}$ 属于吸收状态，因为智能体一旦进入这些状态，交互即告终止。系统设置两个动作：向上（↑）与向下（↓）。奖励函数在除目标状态 $S_{T2}$ 外的所有位置均为零；进入 $S_{T2}$ 时可获得奖励 1。因此，在每个状态下的最优动作都是向下移动。
...
在本节中，我们讨论当 MDP 模型已知时，如何计算最优策略。这个问题被称为**规划**（planning），与之相对的是模型未知情况下的**学习**问题，后者通过强化学习（第35章）来解决。我们将讨论两种规划算法——**动态规划**（dynamic programming,DP） 和**线性规划**（linear programming,LP）。

出于简化，本节我们假设状态和行动的集合是离散的，且 $\gamma<1$。然而，最优策略的精确计算复杂度通常与状态空间 $S$ 和行动空间 $\mathcal{A}$ 的大小成多项式关系，当状态空间是多个有限集合的笛卡尔积时，这个问题通常难以处理。这一挑战被称为**维度灾难**（curse of dimensionality）。因此，通常需要采用近似方法，例如使用价值函数或策略的参数化或非参数化表示，这既是为了计算上的可行性，也是为了将这些方法推广到处理具有一般状态和行动集合的 MDP。在这种情况下，我们有了**近似动态规划**（approximate dynamic programming,ADP） 和**近似线性规划**（approximate linear programming,ALP） 算法（例如，参见 [Ber19]）。

<img src="/assets/img/figures/book2/34.14.png" alt="image-20251221144324670" style="zoom:70%;" />

### 34.6.1 价值迭代

一种普遍使用且有效求解 MDP 的动态规划方法是**价值迭代**（value iteration, VI）。从初始价值函数估计 $V_0$ 开始，该算法通过以下方式迭代更新估计值：

$$
V_{k+1}(s)=\max _a\left[R(s, a)+\gamma \sum_{s^{\prime}} p\left(s^{\prime} \mid s, a\right) V_k\left(s^{\prime}\right)\right] \tag{34.86}
$$

请注意，该更新规则（有时称为**贝尔曼备份**（Bellman backup））正是贝尔曼最优性方程 (34.82) 的右侧，只是用当前估计值 $V_k$ 替换了未知的 $V_*$。方程 (34.86) 的一个基本性质是更新结果是一个**收缩映射**（contraction）：可以证明

$$
\max _s\left|V_{k+1}(s)-V_*(s)\right| \leq \gamma \max _s\left|V_k(s)-V_*(s)\right| \tag{34.87}
$$

换句话说，每次迭代都会将价值函数的最大误差减少一个常数因子。由此可知，$V_k$ 将收敛至 $$V_*$$，并在此基础上得到最优策略。在实践中，当 $$V_k$$足够接近 $$V_*$$ 时，我们便可以终止 VI，因为基于 $$V_k$$ 的贪心策略已经接近最优。价值迭代的思路同样可以用来学习最优行动价值函数 $$Q_*$$。

在价值迭代中需要考虑所有可能状态 $s$ 的 $$V_*(s)$$ 和 $$\pi_*(s)$$，并在每次迭代中对所有可能的下一状态 $$s^\prime$$ 取平均，如图 34.14（右）所示。然而，对于某些问题，我们只关心某些特殊起始状态的价值（和策略）。例如，在图的最短路径问题中，我们试图从当前状态找到到达目标状态的最短路径。这可以通过定义一个转移矩阵 $$p_T\left(s^{\prime} \mid s, a\right)$$ 建模为分幕式 MDP，其中从节点 $$s$$ 延边 $$a$$ 到达相邻节点 $$s^\prime$$ 的概率为 $1$ （确定性转移）。奖励函数定义为对所有状态 $s$（目标状态除外） $$R(s, a)=-1$$，目标状态被建模为吸收状态。
...
这是一个包含 $\mid\mathcal{S}\mid$ 个未知数的线性方程组。我们可以使用矩阵求逆来求解它：$\boldsymbol{v}=(\mathbf{I}-\gamma \mathbf{T})^{-1} \boldsymbol{r}$。或者，我们可以使用价值迭代，通过计算 $\boldsymbol{v}_{t+1}=\boldsymbol{r}+\gamma \mathbf{T} \boldsymbol{v}_t$ 直至接近收敛，或者使用某种计算上更高效的异步变体。

一旦我们评估了当前策略 $\pi$ 的 $V_\pi$，我们就可以用它来推导出一个更好的策略 $\pi^{\prime}$，因此这一步被称为策略改进。为此，我们只需计算一个确定性策略 $\pi^{\prime}$，它在每个状态下都相对于 $V_\pi$ 采取贪心行动；也就是说 $$\pi^{\prime}(s)=\operatorname{argmax}_a\left\{R(s, a)+\gamma \mathbb{E}\left[V_\pi\left(s^{\prime}\right)\right]\right\}$$。我们可以保证 $$V_{\pi^{\prime}} \geq V_\pi$$（逐状态不小于）。为理解这一点，像之前一样定义新策略 $\pi^{\prime}$ 对应的 $$\boldsymbol{r}^{\prime}$$，$$\mathbf{T}^{\prime}$$和 $$\boldsymbol{v}^{\prime}$$ 。根据 $$\pi^{\prime}$$ 的定义可知 $$\boldsymbol{r}^{\prime}+\gamma \mathbf{T}^{\prime} \boldsymbol{v} \geq \boldsymbol{r}+\gamma \mathbf{T} \boldsymbol{v}=\boldsymbol{v}$$，其中等号来自于贝尔曼方程。重复应用同样的等式，我们有
...
当策略 $\pi_k$ 相对于其自身的价值函数 $V_{\pi_k}$ 是贪心的，算法在第 $k$ 次迭代停止。此时，该策略是最优的。由于确定性策略最多有 $$\mid\mathcal{A}\mid^{\mid\mathcal{S}\mid}$$ 种，且每次迭代都会严格改进策略，因此算法必定在有限次迭代后收敛。

在 PI 中，我们在策略评估（涉及多次迭代，直到 $V_\pi$ 收敛）和策略改进之间交替进行。在 VI 中，我们在一次策略评估迭代和一次策略改进迭代（更新规则中的“max”算子）之间交替进行。在**广义策略迭代**（generalized policy improvement）中，我们可以自由地以任意顺序混合任意数量的这些步骤。一旦策略相对于其自身的值函数是贪心的，该过程就会收敛。

注意，策略评估计算的是 $V_\pi$，而价值迭代计算的是 $V_*$。这一差异在图 34.14 中使用**备份图**（backup diagram）进行了说明。这里，根节点代表任意状态 $s$，下一层的节点代表状态-行动组合（实心圆），叶节点代表每个可能行动所导致的下一个状态 $s^{\prime}$的集合。在前一种情况下（策略评估），我们根据策略对所有行动取平均，而在后者（价值迭代）中，我们对所有行动取最大值。

### 34.6.3 线性规划

虽然动态规划有效且广受欢迎，但线性规划（LP）提供了一种替代方法，在某些场景中有重要应用，例如在离线策略强化学习中（第35.5节）。LP的原始形式如下：

$$
\min _V \sum_s p_0(s) V(s) \quad \text { s.t. } \quad V(s) \geq R(s, a)+\gamma \sum_{s^{\prime}} p_T\left(s^{\prime} \mid s, a\right) V(s), \quad \forall(s, a) \in \mathcal{S} \times \mathcal{A} \tag{34.92}
$$

其中对所有 $s \in \mathcal{S}$ 都有 $p_0(s)>0$，可被解释为初始状态分布。可以验证，满足式（34.92）中约束的任意 $V$ 都是**乐观的** [Put94]，即 $$V \geq V_*$$。当目标函数被最小化时，解 $$V$$ 会被“推”到尽可能小的值，也就是 $$V_*$$。一旦找到了 $$V_*$$，在状态 $$s$$ 中使该约束取等号的任何动作 $$a$$ 在该状态下都是最优的。

LP的对偶形式有时更直观：

$$
\max _{d \geq 0} \sum_{s, a} d(s, a) R(s, a) \quad \text { s.t. } \quad \sum_a d(s, a)=(1-\gamma) p_0(s)+\gamma \sum_{\bar{s}, \bar{a}} p_T(s \mid \bar{s}, \bar{a}) d(\bar{s}, \bar{a}) \quad \forall s \in \mathcal{S} \quad \tag{34.93}
$$

满足上述约束的任意非负 $d$ 是某个对应策略 $\pi_d(a \mid s) \triangleq d(s, a) / \sum_{a^{\prime}} d\left(s, a^{\prime}\right)$ 的**归一化占用分布**（normalized occupancy distribution）：

$$
d(s, a)=(1-\gamma) \sum_{t=0}^{\infty} \gamma^t p\left(s_t=s, a_t=a \mid s_0 \sim p_0, a_t \sim \pi_d\left(s_t\right)\right) \tag{34.94}
$$

常数 $(1-\gamma)$ 将 $d$ 归一化为一个合法的分布，使其总和为$1$。通过这种对 $d$ 的解释，式（34.93）中的目标函数就是归一化占用分布下的平均每步奖励。一旦找到最优解 $$d_*$$，最优策略就可以立即由 $$\pi_*(a \mid s)=d_*(s, a) / \sum_{a^{\prime}} d_*\left(s, a^{\prime}\right)$$ 获得。

求解MDP的原始或对偶LP时面临的挑战是大量的约束和变量。需要采用近似方法，其中变量被参数化（线性或非线性），约束被采样或近似（参见例如 [dV04; LBS17; CLW18]）。

## 34.7 主动学习

在本节中，我们将讨论**主动学习**（active learning，AL），在这种学习方式中，智能体可以自主选择它想要使用的数据，以便尽可能快地学习潜在的预测函数，即使用最少数量的标注数据。如图 34.15 所示，这比使用随机收集的数据要高效得多。这在标注成本高昂的场景中非常有用，例如医学图像分类 [GIG17; Wal+20]。

主动学习有多种方法，正如 [Set12; Ren+21; ZSH22] 中所综述的那样。在本节中，我们仅讨论其中几种方法。

<img src="/assets/img/figures/book2/34.15.png" alt="image-20260713234802912" style="zoom:50%;" />

### 34.7.1 主动学习场景

最早的主动学习方法之一被称为**成员查询合成**（membership query synthesis） [Ang88]。在这种场景下，智能体可以生成任意查询 $\boldsymbol{x} \sim p(\boldsymbol{x})$，然后向“神谕”请求其标签 $y=f(\boldsymbol{x})$。（"神谕"是指一个能知晓所有可能问题真实答案的系统。）这种场景主要具有理论上的研究价值，因为很难学习到好的生成模型，而且几乎不可能随时按需访问一个神谕（尽管基于人力的**众包计算**（crowd computing）平台可以被视为具有高延迟的"神谕"）。
...
机器学习中最后一种且被广泛使用的设定是**池式采样**（pool-based-sampling） [LG94]，在这种设定中，未标注样本池 $\mathcal{X}$ 从一开始就可用。在每一步中，我们对批次中的每个候选样本应用**采集函数**(acquisition function)，来决定收集哪个样本的标签。然后我们收集该标签，用新数据更新模型，并重复此过程，直到用尽样本池、时间耗尽或达到期望的性能水平。在后续章节中，我们将只关注池式采样。

<img src="/assets/img/figures/book2/34.16.png" alt="image-20260713234828604" style="zoom:50%;" />

### 34.7.2 与其他形式的序列决策的关系

池式主动学习与贝叶斯优化（第6.6节）和上下文老虎机问题（第34.4节）密切相关。[Tou14] 详细讨论了它们之间的联系，但简而言之，这些方法之所以不同，是因为它们解决的目标函数略有不同，如表34.1所总结。具体来说：在**主动学习**中，我们的目标是识别一个函数 $f: \mathcal{X} \rightarrow \mathcal{Y}$，该函数在应用于随机输入 $\boldsymbol{x}$ 时能产生最小的期望损失。在**贝叶斯优化**中，我们的目标是识别一个输入点 $\boldsymbol{x}$，使得函数输出 $f(\boldsymbol{x})$ 最大。在**老虎机问题**中，我们的目标是识别一个策略 $\pi: \mathcal{X} \rightarrow \mathcal{A}$，使其在应用于随机输入（上下文）$\boldsymbol{x}$ 时产生的期望奖励最大。（我们可以看到，AL和赌博机的目标相似，但在赌博机中，智能体只能选择动作，而不能选择状态，因此对于（奖励）函数在哪里被评估只有部分控制权。）

在这三个问题中，我们都希望用尽可能少的行动来找到最优解，因此必须解决探索-利用的权衡问题（第34.4.3节）。一种方法是使用诸如高斯过程（第18章）之类的方法来表示我们对函数的不确定性，这让我们能够计算 $$p\left(f \mid \mathcal{D}_{1: t}\right)$$。然后我们定义某个**采集函数** $$\alpha(\boldsymbol{x})$$，该函数在给定信念状态 $$p\left(f \mid \mathcal{D}_{1: t}\right)$$ 的情况下，评估在输入位置 $$\boldsymbol{x}$$ 查询函数的有用程度，并选择 $$\boldsymbol{x}_{t+1}=\operatorname{argmax}_{\boldsymbol{x}} \alpha(\boldsymbol{x})$$ 作为我们的下一个查询点。（在老虎机设置中，智能体无法选择状态 $$\boldsymbol{x}$$，但可以选择行动 $$a$$。）例如，在贝叶斯优化中，通常使用改进概率（第6.6.3.1节），而对于回归任务的主动学习，我们可以使用后验预测方差。主动学习的目标会使智能体“到处”查询，而贝叶斯优化则会让智能体“聚焦”在最有可能的区域，如图34.16所示。我们将在第34.7.3节中讨论主动学习的其他采集函数。

<img src="/assets/img/figures/book2/34.17.png" alt="image-20260713234905236" style="zoom:50%;" />
...
一种直观的、用于选择下一个标注样本的启发式方法是：挑选模型当前最不确定的那个样本。这被称为**不确定性采样**（uncertainty sampling）。我们已经在图34.16的回归问题中说明了这一点，其中我们用后验方差来表示不确定性。

对于分类问题，我们可以用多种方式衡量不确定性。令 $$\boldsymbol{p}_n=\left[p\left(y=c \mid \boldsymbol{x}_n\right)\right]_{c=1}^C$$ 表示每个未标注输入 $$\boldsymbol{x}_n$$ 的类别概率向量。令 $$U_n=\alpha\left(\boldsymbol{p}_n\right)$$ 表示样本 $$n$$ 的不确定性，其中 $$\alpha$$ 是表示采集函数。一些常见的 $$\alpha$$ 选择包括：**熵采样**（entropy sampling） [SW87a]，使用 $$\alpha(\boldsymbol{p})=-\sum_{c=1}^C p_c \log p_c$$；**间隔采样**（margin sampling），使用 $$\alpha(\boldsymbol{p})=p_2-p_1$$，其中 $p_1$ 是最可能类别的概率，$p_2$ 是第二可能类别的概率；**最不置信采样**（least confident sampling），使用 $\alpha(\boldsymbol{p})=1-p_c$.，其中 $c^*=\operatorname{argmax}_c p_c$。这些策略之间的差异如图34.17所示。在实践中，通常发现间隔采样效果最好 [Chu+19]。

<img src="/assets/img/figures/book2/34.18.png" alt="image-20260713235048057" style="zoom:50%;" />
...
一种自然的采集策略是选择那些其标签能最大程度减少我们关于模型参数 $\boldsymbol{w}$ 的不确定性的数据点。这被称为**信息增益**（information gain）准则，最早由 [Lin56] 提出。其定义如下：
$$
...
$$
\alpha^{\prime}(\boldsymbol{x}) \triangleq \mathbb{E}_{p(y \mid \boldsymbol{x}, \mathcal{D})}\left[D_{\mathbb{KL}}(p(\boldsymbol{w} \mid \mathcal{D}, \boldsymbol{x}, y) \| p(\boldsymbol{w} \mid \mathcal{D}))\right] \tag{34.96}
$$
...
\end{align}
$$
这种方法的优点在于，我们现在只需要考虑输出 $y$ 的预测分布的不确定性，而不必考虑参数 $\boldsymbol{w}$ 的不确定性。这种方法被称为**基于分歧的贝叶斯主动学习** （Bayesian active learning by disagreement，BALD）。[Hou+12]。

方程 (34.99) 有一个有趣的解释。第一项偏好那些预测标签存在不确定性的样本 $\boldsymbol{x}$。仅使用此项作为选择标准相当于我们上面讨论过的**不确定性采样**。然而，这对于本身存在歧义或错误标注的样本可能会有问题。通过加入第二项，我们会对这种行为进行惩罚，因为对于那些即使已知参数后预测分布仍然具有高熵的样本点，我们加上了一个大的负权重。因此，我们忽略了**偶然性（固有的）不确定性**，而专注于**认知不确定性**。

<img src="/assets/img/figures/book2/34.19.png" alt="image-20260714233225548" style="zoom:50%;" />

### 34.7.4 Batch 主动学习

在许多应用中，我们需要一次性选择一批未标注的样本，因为针对单个样本逐一训练模型太慢了。这被称为**批量主动学习（batch active learning）**。其关键挑战在于，我们需要确保所请求的不同查询是**多样化的**，从而最大化信息增益。针对这一问题已经设计了多种方法；这里我们重点介绍 [KAG19] 提出的 **BatchBALD** 方法，它扩展了第 34.7.3.3 节中的 BALD 方法。

#### 34.7.4.1 BatchBALD

将BALD分数扩展到由 $b$ 个候选查询点组成的批次的朴素方式是定义

$$
\alpha_{\mathrm{BALD}}\left(\left\{\boldsymbol{x}_1, \ldots, \boldsymbol{x}_B\right\}, p(\boldsymbol{w} \mid \mathcal{D})\right)=\alpha_{\mathrm{BALD}}\left(\boldsymbol{x}_{1: B}, p(\boldsymbol{w} \mid \mathcal{D})\right)=\sum_{i=1}^B \mathbb{I}\left(y_i ; \boldsymbol{w} \mid \boldsymbol{x}_i, \mathcal{D}\right) \tag{34.100}
$$

然而，这种方式可能会选取在信息内容方面相当相似的样本点。在BatchBALD中，我们使用标签集合与参数之间的**联合条件互信息**：

$$
\alpha_{\text {BBALD }}\left(\boldsymbol{x}_{1: B}, p(\boldsymbol{w} \mid \mathcal{D})\right)=\mathbb{I}\left(\boldsymbol{y}_{1: B} ; \boldsymbol{w} \mid \boldsymbol{x}_{1: B}, \mathcal{D}\right)=\mathbb{H}\left(\boldsymbol{y}_{1: B} \mid \boldsymbol{x}_{1: B}, \mathcal{D}\right)-\mathbb{E}_{p(\boldsymbol{w} \mid \mathcal{D})}\left[\mathbb{H}\left(\boldsymbol{y}_{1: B} \mid \boldsymbol{x}_{1: B}, \boldsymbol{w}, \mathcal{D}\right)\right] \tag{34.101}
$$

为了理解这与BALD有何不同，我们将使用信息图以维恩图的形式表示互信息，如第5.3.2节所述。特别是，[Yeung91] 表明，对于离散随机变量 $x$ 和 $y$，我们可以定义一个符号测度 $$\mu^*$$，使得 $$\mathbb{I}(x ; y)=\mu^*(x \cap y)$$，$$\mathbb{H}(x, y)=\mu^*(x \cup y)$$，$$\mathbb{E}_{p(y)}[\mathbb{H}(x \mid y)]=\mu^*(x \backslash y)$$，等等。利用这一点，我们可以将标准BALD解释为各个交集之和 $$\sum_i \mu^*\left(y_i \cap \boldsymbol{w}\right)$$，这会**重复计算** $y_i$ 之间的重叠部分，如图34.19(a)所示。相比之下，BatchBALD通过计算

$$
\mathbb{I}\left(\boldsymbol{y}_{1: B} ; \boldsymbol{w} \mid \boldsymbol{x}_{1: B}, \mathcal{D}\right)=\mu^*\left(\cup_i y_i \cap \boldsymbol{w}\right)=\mu^*\left(\cup_i y_i\right)-\mu^*\left(\cup_i y_i \backslash \boldsymbol{w}\right) \tag{34.102}
$$

来考虑重叠。如图34.19(b)所示。由此可以看出，$\alpha_{\mathrm{BBALD}} \leq \alpha_{\mathrm{BALD}}$。实际上，可以证明

$$
\mathbb{I}\left(\boldsymbol{y}_{1: B}, \boldsymbol{w} \mid \boldsymbol{x}_{1: B}, \mathcal{D}\right)=\sum_{i=1}^B \mathbb{I}\left(y_i, \boldsymbol{w} \mid \boldsymbol{x}_{1: B}, \mathcal{D}\right)-\mathrm{TC}\left(\boldsymbol{y}_{1: B} \mid \boldsymbol{x}_{1: B}, \mathcal{D}\right) \tag{34.103}
$$

其中TC是**总相关性**（参见第5.3.5.1节）。

#### 34.7.4.2 优化BatchBALD

为了避免因对点集的子集进行联合评分而产生的组合爆炸，我们可以使用一种贪心近似方法来逐点计算BatchBALD。具体而言，假设在第 $n-1$ 步我们已经有了一个部分批次 $\mathcal{A}_{n-1}$。下一个点的选择使用

$$
\boldsymbol{x}_n=\underset{\boldsymbol{x} \in \mathcal{D}_{\text {pool }} \backslash \mathcal{A}_{n-1}}{\operatorname{argmax}} \alpha_{\text {BBALD }}\left(\mathcal{A}_{n-1} \cup\{\boldsymbol{x}\}, p(\boldsymbol{w} \mid \mathcal{D})\right) \tag{34.104}
$$

然后我们将 $$\boldsymbol{x}_n$$ 加入 $$\mathcal{A}_{n-1}$$ 得到 $\mathcal{A}_n$。幸运的是，如 [KAG19] 所示，BatchBALD采集函数是**子模的**。因此，这个贪心算法的最优性保证在 $$1-1 / e \approx 0.63$$ 之内（参见第6.9.4.1节）。

#### 34.7.4.3 计算BatchBALD

计算联合（条件）互信息是难以处理的，因此在本节中，我们讨论如何对其进行近似。为简洁起见，我们省略对 $\boldsymbol{x}$ 和 $\mathcal{D}$ 的条件依赖。使用这种新符号，目标函数变为

$$
\alpha_{\text {BBALD }}\left(\boldsymbol{x}_{1: B}, p(\boldsymbol{w} \mid \mathcal{D})\right)=\mathbb{H}\left(y_1, \ldots, y_B\right)-\mathbb{E}_{p(\boldsymbol{w})}\left[\mathbb{H}\left(y_1, \ldots, y_B \mid \boldsymbol{w}\right)\right] \tag{34.105}
$$

注意，在给定 $\boldsymbol{w}$ 的条件下，各个 $$y_i$$ 是条件独立的，因此 $\mathbb{H}\left(y_1, \ldots, y_B \mid \boldsymbol{w}\right)=\sum_{i=1}^B \mathrm{H}\left(y_i \mid \boldsymbol{w}\right)$。因此我们可以用蒙特卡洛来近似第二项：

$$
\mathbb{E}_{p(\boldsymbol{w})}\left[\mathbb{H}\left(y_1, \ldots, y_B \mid \boldsymbol{w}\right)\right] \approx \frac{1}{S} \sum_{i=1}^n \sum_s \mathbb{H}\left(y_i \mid \hat{\boldsymbol{w}}_s\right) \tag{34.106}
$$

其中 $\hat{\boldsymbol{w}}_s \sim p(\boldsymbol{w} \mid \mathcal{D})$。

第一项 $\mathbb{H}\left(y_1, \ldots, y_B\right)$ 是联合熵，因此计算起来更困难。[KAG19] 提出了以下近似方法，对批次中所有可能的标签序列进行求和，并利用 $p(\boldsymbol{y})=\mathbb{E}_{p(\boldsymbol{w})}[p(\boldsymbol{y} \mid \boldsymbol{w})]$ 这一事实：

$$
\begin{align}
\mathbb{H}\left(\boldsymbol{y}_{1: B}\right) & =\mathbb{E}_{p(w) p\left(\boldsymbol{y}_{1: B} \mid v w\right)}\left[-\log p\left(\boldsymbol{y}_{1: B} \mid \boldsymbol{w}\right)\right] \tag{34.107}\\
& \approx \sum_{\hat{\boldsymbol{y}}: 1:}\left(\frac{1}{S} \sum_{s=1}^S p\left(\hat{\boldsymbol{y}}_{1: B} \mid \hat{\boldsymbol{w}}_s\right)\right) \log \left(\frac{1}{S} \sum_{s=1}^S p\left(\hat{\boldsymbol{y}}_{1: B} \mid \hat{\boldsymbol{w}}_s\right)\right) \tag{34.108}
\end{align}
$$

对所有可能标签序列的求和可以通过以下方式提高效率：注意到 $$p\left(\boldsymbol{y}_{1: m} \mid \boldsymbol{w}\right)=p\left(y_n \mid \boldsymbol{w}\right) p\left(\boldsymbol{y}_{1: n-1} \mid \boldsymbol{w}\right)$$，因此当我们实现贪心算法时，可以增量式地更新概率，重用之前的计算结果。详见 [KAG19]。

#### 34.7.4.4 BALD 和 BatchBALD在MNIST上的实验对比

在本节中，我们展示了将BALD和BatchBALD应用于标准MNIST数据集上训练CNN的一些实验结果。我们使用的批次大小为4，并使用MC dropout（第17.3.1节）来近似参数的后验分布 $p(\boldsymbol{w} \mid \mathcal{D})$。在图34.20(a)中，我们可以看到BALD选择的样本彼此非常相似，而在图34.20(b)中，我们可以看到BatchBALD选择了更多样化的样本点。在图34.20(c)中，我们可以看到BatchBALD比BALD学习效率更高，而BALD又比随机采样数据的学习效率更高。
