...
- 生成模型的训练主要包含2种范式：**likelihood-based 方法** 和 **生成对抗网络**。前者使用对数似然（或者易处理的代理）作为优化目标，后者使用对抗训练的方案最小化模型预测分布与真实数据分布之间的差异，如 $f$-散度；
-  likelihood-based 方法（如自回归模型，归一化流模型）在模型结构和优化目标上存在限制；GAN 的训练存在不稳定的情况，同时GAN模型不适合评估和对比不同模型（无法计算样本的概率值，即$p(\mathbf{x})$）。
- 作者提出一种新的数据生成的建模方案 <u>Score-based generative models</u>，主要包括：
...
<img src="/Users/fengliangqi/翻译/images/流形假设导致梯度无定义.png" alt="image-20241129180452425" style="zoom:50%;" />
...
其次，根据 [Hyv¨arinen][^Hyv¨arinen] 中的定理2，只有当概率分布的支撑集为整个空间（数据空间没有空洞），score matching估计的 score 才是一致的，如果数据被约束在某个低维的流形，会导致不一致的 score。如下图所示，如果存在稀疏的区域，那可能存在多个概率模型参数 $\theta^{\prime}$ 或 $\theta^{\prime \prime}$ 满足数据的分布，而此时在 $x$ 处的score则出现不一致，随着数据量的增加， $\theta^{\prime}$ 或 $\theta^{\prime \prime}$ 将趋于一致，相应的每一点的score也具有了一致性。

[^Hyv¨arinen]: $\textrm{[Hyv¨arinen]}$: Estimation of non-normalized statistical models by score matching. Journal of Machine Learning Research, 6(Apr):695–709, 2005.

<img src="/Users/fengliangqi/翻译/images/数据稀疏导致score估计出现不一致.jpg" alt="画作 2" style="zoom:20%;" />

#### 1.2.4 低密度数据区域带来的挑战

回顾公式（1.1），denoising score matching 需要关于 $p_{\text {data }}(\mathbf{x})$ 求解期望。在实践中，为了估计期望值，首先采样得到样本 $\left\{\mathbf{x}_i\right\}_{i=1}^N \stackrel{\text { i.i.d. }}{\sim} p_{\text {data }}(\mathbf{x})$，考虑某个低密度区域 $\mathcal{R} \subset \mathbb{R}^D$，该区域满足 $p_{\text {data }}(\mathcal{R}) \approx 0$。在多数情况下 $\left\{\mathbf{x}_i\right\}_{i=1}^N \cap \mathcal{R}=\varnothing$，即我们很少采样到那些低密度区域的样本。此时，期望的估计会严重失准，进而导致低密度区域的 score 不再可靠。

<img src="/Users/fengliangqi/翻译/images/低密度区域影响score估计.png" alt="image-20241129182847881" style="zoom: 33%;" />
...
<img src="/Users/fengliangqi/翻译/images/低密度区域对采样的影响.png" alt="image-20241129190035524" style="zoom:45%;" />
...
<img src="/Users/fengliangqi/翻译/images/退火朗之万采样算法.png" alt="image-20241129220654028" style="zoom:60%;" />
...
<img src="/Users/fengliangqi/翻译/images/退火朗之万动力学捕捉混合权重.jpg" alt="画作 2" style="zoom:15%;" />
...
![image-20241129225115714](/Users/fengliangqi/翻译/images/score-based模型可视化效果.png)

<img src="/Users/fengliangqi/翻译/images/退火朗之万生成的中间结果.png" alt="image-20241129225226826" style="zoom:50%;" />

上图展示了退火朗之万采样器的中间结果，不难发现样本从完全噪声到正常图片的演变过程。

#### 1.3.2 定量分析

<img src="/Users/fengliangqi/翻译/images/score-based结果的定量分析.png" alt="image-20241129225507631" style="zoom:50%;" />
...
在基础的 score-based模型中，作者使用了多层加噪的策略弥补训练数据分布稀疏的问题。推荐的加噪强度  $\left\{\sigma_i\right\}_{i=1}^L$ 在低分辨率 $32 \times 32$ 上表现良好，但不适用于更高分辨率的图片。其次，朗之万动力学采样策略在高分辨率图片上的生成表现也不佳。
...
假设真实数据分布 $p_{\text {data }}(\mathbf{x})$ 中的独立同分布样本为 $\left\{\mathbf{x}^{(1)}, \mathbf{x}^{(2)}, \cdots, \mathbf{x}^{(N)}\right\}$。如果 $N$ 足够大，我们有 $p_{\text {data }}(\mathbf{x}) \approx \hat{p}_{\text {data }}(\mathbf{x}) \triangleq \frac{1}{N} \sum_{i=1}^N \delta\left(\mathbf{x}=\mathbf{x}^{(i)}\right)$。如果我们使用噪声 $\mathcal{N}\left(\mathbf{0}, \sigma_1^2 \mathbf{I}\right)$ 对样本进行扰动，最后一层的经验分布为 $\hat{p}_{\sigma_1}(\mathbf{x}) \triangleq \frac{1}{N} \sum_{i=1}^N p^{(i)}(\mathbf{x})$，其中 $p^{(i)}(\mathbf{x}) \triangleq \mathcal{N}\left(\mathbf{x} \mid \mathbf{x}^{(i)}, \sigma_1^2 \mathbf{I}\right)$。我们==需要避免初始化对样本生成的多样性有影响==，也就是说，不管我们的采样起点在哪，最终的样本可以到达任意一个真实数据。形式化的表述为，假设我们从分布 $p^{(j)}(\mathbf{x})$ 初始化采样起点，我们希望采样的终点依然有可能是样本 $\mathbf{x}_i$，其中 $i \neq j$。而采样的路径变化由 score function $\nabla_{\mathbf{x}} \log \hat{p}_{\sigma_1}(\mathbf{x})$ 决定。
...
$$
其中 $C>0$ 对于所有的 $1<i \leq L$ 满足。$\gamma_i \triangleq \sigma_{i-1} / \sigma_i$，$\Phi(\cdot)$ 表示标准高斯分布的CDF（累积概率函数）。这个选择意味着 $\gamma_2=\gamma_3=\cdots \gamma_L$，所以 $\left\{\sigma_i\right\}_{i=1}^L$ 是一个几何序列。理想状态下，我们希望设置足够多的加噪层数使得 $C \approx 1$。然而，太多的加噪层会增加推理的时间成本。另一方面，在原先的设置中，$L=10$（对于$32 \times 32$图像）可以说太小了。为了达到平衡，作者令 $C \approx 0.5$。

**技巧2**：（中间加噪尺度）选择 $\left\{\sigma_i\right\}_{i=1}^L$ 为几何序列，相邻尺度比例为常数 $\gamma$，该常数满足 $
...
$$
根据**技巧2**，$\left\{\sigma_i\right\}_{i=1}^L$ 是一个几何序列。考虑到 $\gamma$ 在不同噪声层是共享的，所以 $s_T^2 / \sigma_i^2$ 在不同的加噪层是相等的。进一步，$s_T^2 / \sigma_i^2$ 的大小与维度 $D$ 无显式依赖关系。
...
<img src="/Users/fengliangqi/翻译/images/NCSNv2与NCSN的中间结果对比.png" alt="image-20241201181508784" style="zoom:50%;" />

上图展示了每训练 5000 步在1000个样本上的 FIDs（包含 denoising 步骤），其中 denoising 步骤是指，在原有的朗之万采样结尾，新增一步降噪的步骤，如下图展示的算法，第9行可以额外提高 FIDs。

<img src="/Users/fengliangqi/翻译/images/朗之万动力学采样w-denoising.png" alt="image-20241201181943373" style="zoom:40%;" />

更多样本上的测试结果为：

<img src="/Users/fengliangqi/翻译/images/NCSNv2与NCSN的最终对比结果.png" alt="image-20241201182207890" style="zoom:40%;" />
...
<img src="/Users/fengliangqi/翻译/images/SDE示意图.png" alt="image-20241201192756940" style="zoom:45%;" />
...
在DDPM中，同样预设了一个加噪序列 $0<\beta_1, \beta_2, \cdots, \beta_N<1$。对于每一个训练样本 $\mathbf{x}_0 \sim p_{\text {data }}(\mathbf{x})$，构造一个离散马尔可夫链 $\left\{\mathbf{x}_0, \mathbf{x}_1, \cdots, \mathbf{x}_N\right\}$，马尔可夫的状态转移函数定义为 $p\left(\mathbf{x}_i \mid \mathbf{x}_{i-1}\right)=\mathcal{N}\left(\mathbf{x}_i ; \sqrt{1-\beta_i} \mathbf{x}_{i-1}, \beta_i \mathbf{I}\right)$，因此我们有 $p_{\alpha_i}\left(\mathbf{x}_i \mid \mathbf{x}_0\right)=\mathcal{N}\left(\mathbf{x}_i ; \sqrt{\alpha_i} \mathbf{x}_0,\left(1-\alpha_i\right) \mathbf{I}\right)$，其中 $\alpha_i:=\prod_{j=1}^i\left(1-\beta_j\right)$。类似于 SMLD，DDPM 中的扰动分布为 $p_{\alpha_i}(\tilde{\mathbf{x}}):=\int p_{\text {data }}(\mathbf{x}) p_{\alpha_i}(\tilde{\mathbf{x}} \mid \mathbf{x}) \mathrm{d} \mathbf{x}$。
...
<img src="/Users/fengliangqi/翻译/images/预测-矫正采样器框架.png" alt="image-20241202173532147" style="zoom:45%;" />
...
![image-20241202173712166](/Users/fengliangqi/翻译/images/PCSampler_VE_VP.png)

上图展示了不同SDE下的PC（predictor-corrector）采样器的伪代码。对应的corrector 部分如下图所示：

![image-20241202173958469](/Users/fengliangqi/翻译/images/corrector_VE_VP.png)

关于不同采样器的性能对比，我们将在介绍完 PROBABILITY FLOW 预测器之后统一给出。

#### 3.2.4 PROBABILITY FLOW

作者认为，在 score-based 模型中，对于所有前向扩散过程，存在一个对应的 deterministic（确定的） 扩散过程，该确定过程所对应的轨迹与SDE 轨迹共享不同时刻的边际概率分布 $\left\{p_t(\mathbf{x})\right\}_{t=0}^T$。这个确定过程满足一个常微分方程（ODE）：
$$
...
![image-20241202180156576](/Users/fengliangqi/翻译/images/SDE框架下的scorebased模型.png)
...
<img src="/Users/fengliangqi/翻译/images/概率流采样插值.png" alt="image-20241202230248338" style="zoom:44%;" />
...
![image-20241203103236403](/Users/fengliangqi/翻译/images/ODE编码的唯一性.png)

**高效采样** 考虑到 probability flow ODE 是一种 neural ODEs，所以我们可以用一个 black-box ODE solver 生成样本，同时允许我们在精度和效率之间进行权衡。

<img src="/Users/fengliangqi/翻译/images/ODE采样精度与效率的权衡.png" alt="image-20241202235842508" style="zoom:45%;" />
...
![image-20241203110901359](/Users/fengliangqi/翻译/images/PC采样器的性能量化对比.png)
...
<img src="/Users/fengliangqi/翻译/images/SDE结果量化对比.png" alt="image-20241203144856275" style="zoom:50%;" />
...
<img src="/Users/fengliangqi/翻译/images/一致性蒸馏伪代码.png" alt="image-20241204150718046" style="zoom:50%;" />
...
<img src="/Users/fengliangqi/翻译/images/一致性训练伪代码.png" alt="一致性训练伪代码" style="zoom:50%;" />

#### 4.2.5 超参数选择

根据式（4.9），一致性训练与一致性蒸馏高度相关，所以在相关参数选择上，作者基于蒸馏方法进行确定。在一致性蒸馏的训练过程中，存在 度量函数 $d(\cdot, \cdot)$ ，ODE solver 以及离散步数$N$ 的超参数选择。

![image-20241204162451020](/Users/fengliangqi/Library/Application Support/typora-user-images/image-20241204162451020.png)
...
![image-20241204175109324](/Users/fengliangqi/翻译/images/CD和CT训练超参数选择.png)
...
![image-20241204175621289](/Users/fengliangqi/翻译/images/蒸馏结果的性能对比.png)
...
![image-20241204180251443](/Users/fengliangqi/翻译/images/CT在单步生成的性能对比标题.png)

![image-20241204180133223](/Users/fengliangqi/翻译/images/CT在单步生成的性能对比.png)
